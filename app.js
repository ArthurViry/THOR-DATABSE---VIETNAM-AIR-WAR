...
async function loadData(){
  ...
  // The exported JSON is already pre-filtered by the extraction script
  // (Linebacker II dates, study-area coordinates, and WEAPONTYPEWEIGHT >= 1).
  // Keep a defensive weight check here in case the source file changes later.
...
function initMap(){
  ...
  baseTile=L.tileLayer(BASEMAPS.positron.url,{attribution:BASEMAPS.positron.attr,subdomains:'abcd',maxZoom:19}).addTo(map);
  initLasso();
}
...
function _basePieHTML(records){
  ...
  if(!total) return '<div style="font-family:JetBrains Mono,monospace;font-size:10px;color:#5a7888;padding:6px 0">No armed mission data</div>';
  ...
  if(S.layers.points){
    ...
    groups.forEach(g => {
      ...
      hit.on('mouseover', e => showTT(`
        ...
        <div class="t-sep"></div>
        <div class="tr"><span class="tr-l">Estimated payload Σ</span><span class="tr-v">${fmtN(g.totalWt)} lbs</span></div>
...
const LEG_TT  = `font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600;letter-spacing:.14em;color:var(--text-dim);text-transform:uppercase;margin-bottom:12px;display:block;`;
...
function _updateSizeLegend(){
  ...
  el.innerHTML=`<span style="${LEG_TT}">Estimated payload</span>`
    +steps.map(s=>{const r=Math.round(ptRadius(s.wt));const b=r*2+4;
      return`<div style="display:flex;align-items:center;gap:9px;margin-bottom:6px">
        ...
        <span style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text-dim)">${s.l}</span>
      </div>`;}).join('');
  ...
  el.innerHTML=`<span style="${LEG_TT}">Strike routes · aircraft count</span>
    <div style="display:flex;flex-direction:column;gap:7px">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <span style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text-dim)">${naMin} aircraft</span>
      </div>
      <div style="display:flex;align-items:center;gap:10px">
        <svg width="44" height="12"><line x1="0" y1="6" x2="44" y2="6" stroke="#4a9aff" stroke-width="3" stroke-linecap="round"/></svg>
        <span style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text-dim)">~${mid} aircraft</span>
      </div>
      <div style="display:flex;align-items:center;gap:10px">
        <svg width="44" height="18"><line x1="0" y1="9" x2="44" y2="9" stroke="#4a9aff" stroke-width="6" stroke-linecap="round"/></svg>
        <span style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text-dim)">${naMax} aircraft</span>
      </div>
    </div>`;
  // push size legend up
  setTimeout(()=>{
...
function _showBaseLegend(){
  ...
  el.style.display='block';
  el.innerHTML=`<span style="${LEG_TT}">Takeoff airbases</span>
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <span style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text-dim)">Takeoff airbase (click for details)</span>
    </div>`;
  setTimeout(()=>{
...
function updateColorLegend(){
  ...
  S.colorScale.domain().slice(0,10).forEach(v=>{
    html+=`<div style="display:flex;align-items:center;gap:9px;margin-bottom:6px;font-size:13px;color:var(--text)"><div style="width:10px;height:10px;border-radius:50%;background:${S.colorScale(v)};flex-shrink:0"></div>${v}</div>`;
  });
  el.innerHTML=html;
}
...
let _animIsLight = true;   // animated map now defaults to the light basemap
let _animMemoryMode = false; // when true: only show flash, no persistent red dots
function _animFlashColor(){
  return _animIsLight ? '#f08c2e' : '#ffb15a';
}
...
function _buildTonnageChart(){
  ...
  if(!wrap) return;
  const W = wrap.clientWidth || 520, H = 178;
  const pl=50, pr=8, pt=12, pb=30;
...
function _updateChartNeedle(elapsed){
  ...
  const pl=50, pr=8;
  const W1 = document.getElementById('anim-tonnage-chart')?.clientWidth || 520;
  const W2 = document.getElementById('anim-cumul-chart')?.clientWidth || 520;
...
function initAnimMap(){
  ...
  _animIsLight = true;
  L.tileLayer(BASEMAPS.positron.url,{subdomains:'abcd',maxZoom:19}).addTo(animMap);
...
function renderWpn(){
  ...
  rows.forEach(r=>{const c=document.createElement('div');c.className='gc';c.innerHTML=`<div class="gc-top"><div><div class="gc-root">${r.cn||r.wt}</div><div class="gc-name" style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text-dim);margin-top:3px">${r.wt}</div>${r.desc?`<div class="gc-type">${r.desc.slice(0,90)}${r.desc.length>90?'…':''}</div>`:''}</div><span class="wc wc-${r.wc}">${r.wc}</span></div><div class="gc-foot"><span class="gc-uses">${(+r.cnt).toLocaleString('en-US')} uses</span></div>`;grid.appendChild(c);});
}
...
function buildInfographicChronology(){
  const wrap=document.getElementById('infog-chronology');
  const note=document.getElementById('infog-chronology-note');
  if(!wrap||!S.allData.length) return;

  const days=LB2_DAYS.map(day=>{
    const recs=S.allData.filter(d=>d._day===day);
    return {
      day,
      label:String(day).padStart(2,'0'),
      missions:recs.length,
      tonnage:d3.sum(recs,d=>d._wt)/1000
    };
  });
  const peak=days.reduce((best,d)=>d.tonnage>best.tonnage?d:best,days[0]);
  const maxM=d3.max(days,d=>d.missions)||1;
  const maxT=d3.max(days,d=>d.tonnage)||1;
  const avgM=d3.mean(days,d=>d.missions)||0;
  const totalT=d3.sum(days,d=>d.tonnage)||0;

  setText('infog-stat-missions', fmtN(S.allData.length));
  setText('infog-stat-tonnage', fmtN(totalT,1));
  setText('infog-stat-peak-day', `${peak.label} Dec`);
  setText('infog-stat-peak-copy', `${fmtN(peak.missions)} missions · ${fmtN(peak.tonnage,1)} t dropped`);
  setText('infog-stat-avg-missions', fmtN(avgM,1));
  setText('infog-stat-bases', Object.keys(BASE_STATS).length);

  if(note){
    note.innerHTML = `<strong>Operational peak:</strong> <span class="peak-day">${peak.label} December</span> concentrates the highest daily tonnage, with ${fmtN(peak.missions)} armed missions and ${fmtN(peak.tonnage,1)} tons dropped.`;
  }

  wrap.innerHTML='';
  const W=wrap.clientWidth||960;
  const H=Math.max(320, wrap.clientHeight||380);
  const m={top:20,right:76,bottom:46,left:56};
  ...
  const x=d3.scaleBand().domain(days.map(d=>d.day)).range([m.left,m.left+iw]).padding(0.22);
  const yM=d3.scaleLinear().domain([0,maxM*1.15]).nice().range([m.top+ih,m.top]);
  const yT=d3.scaleLinear().domain([0,maxT*1.15]).nice().range([m.top+ih,m.top]);

  const plot=svg.append('g');

  const peakX=x(peak.day)||m.left;
  plot.append('rect')
    .attr('x',peakX-6)
    .attr('y',m.top)
    .attr('width',x.bandwidth()+12)
    .attr('height',ih)
    .attr('fill','rgba(160,58,58,0.12)')
    .attr('stroke','#a03a3a')
  ...
  yM.ticks(4).forEach(t=>{
    ...
    plot.append('line')
      ...
      .attr('y1',y).attr('y2',y)
      .attr('stroke','#1e3050').attr('stroke-width',1);
    plot.append('text')
      .attr('x',m.left-10).attr('y',y+4)
  ...
  plot.selectAll('rect.bar').data(days).enter().append('rect')
    .attr('x',d=>x(d.day))
    .attr('y',d=>yM(d.missions))
    .attr('width',x.bandwidth())
    .attr('height',d=>m.top+ih-yM(d.missions))
    .attr('rx',2)
    .attr('fill','#4e7ea8');
  ...
  plot.selectAll('circle.pt').data(days).enter().append('circle')
    .attr('cx',d=>(x(d.day)||0)+x.bandwidth()/2)
    .attr('cy',d=>yT(d.tonnage))
    .attr('r',d=>d.day===peak.day?5.4:4)
    .attr('fill',d=>d.day===peak.day?'#a03a3a':'#c07830');

  days.forEach(d=>{
    const cx=(x(d.day)||0)+x.bandwidth()/2;
    plot.append('text')
      .attr('x',cx)
      .attr('y',m.top+ih+18)
      .attr('text-anchor','middle')
      .attr('fill',d.day===peak.day?'#c4d4e0':'#6a9ab8')
      .attr('font-size',10)
      .text(d.label);
  });
  ...
  yT.ticks(4).forEach(t=>{
    const y=yT(t);
    plot.append('text')
      .attr('x',m.left+iw+10).attr('y',y+4)
  ...
  plot.append('text')
    ...
    .attr('fill','#4e7ea8').attr('font-size',10)
    .attr('letter-spacing','.12em').text('MISSIONS');
  plot.append('text')
    .attr('x',m.left+iw).attr('y',m.top-6)
    .attr('text-anchor','end')
    .attr('fill','#c07830').attr('font-size',10)
    .attr('letter-spacing','.12em').text('TONNAGE');
}

function renderInfographicTargets(){
  const host=document.getElementById('infog-targets-list');
  if(!host||!S.allData.length) return;
  const rows=Array.from(d3.rollup(
    S.allData,
    vals=>({count:vals.length,cat:getTgtCat(vals[0]?.tt||'OTHER')}),
    d=>(d.tt||'Unknown target').trim()
  ), ([name,meta])=>({name,count:meta.count,cat:meta.cat}))
    .sort((a,b)=>d3.descending(a.count,b.count))
    .slice(0,5);

  host.innerHTML='';
  rows.forEach(r=>{
    const catClass = r.cat==='MILITARY' ? 'mil' : r.cat==='INFRASTRUCTURE' ? 'infra' : /RAIL/.test(r.cat) ? 'rail' : 'other';
    const row=document.createElement('div');
    row.className='infog-tgt-row';
    row.innerHTML=`<div class="itgt-name"><span>${r.name}</span></div><div class="itgt-cat ${catClass}">${r.cat}</div><div class="itgt-sorties">${fmtN(r.count)}</div>`;
...
function showPage(id){
  ...
  if(id==='infog')setTimeout(()=>{buildInfographicChronology();renderInfographicTargets();},80);
...
window.addEventListener('resize',()=>{clearTimeout(window._rt);window._rt=setTimeout(()=>{if(map)map.invalidateSize();if(animMap)animMap.invalidateSize();updateStats();buildInfographicChronology();renderInfographicTargets();},260);});
...
(async function init(){
  try{
    ...
    setText('kpi-days',    LB2_DAYS.length);
    setText('is-missions', fmtN(S.allData.length));
    setText('is-days',     LB2_DAYS.length);
    setText('is-tonnes',   fmtN(totalWt/1000));
    setText('is-bases',    Object.keys(BASE_STATS).length);
    initMap();updateMap();updateStats();buildInfographicChronology();renderInfographicTargets();
